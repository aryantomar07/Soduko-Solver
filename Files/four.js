...
var SudokuSolver = function (testable) {
    ...
    function getNextCellAndPossibilities(boardArray) {
        for (var i = 0; i < boardArray.length; i++) {
            if (boardArray[i] === "-") {
                ...
                var choices = ["1", "2", "3", "4"].filter(function (num) {
    ...
    function allRowsValid(boardArray) {
        return [0, 4, 8, 12].map(function (i) {
    ...
    function getRow(boardArray, i) {
        var startingEl = Math.floor(i / 4) * 4;
        return boardArray.slice(startingEl, startingEl + 4);
    }
    ...
    function getColumn(boardArray, i) {
        var startingEl = Math.floor(i % 4);
        return [0, 1, 2, 3].map(function (num) {
            return boardArray[startingEl + num * 4];
        });
    }

    function allBoxesValid(boardArray) {
        return [0, 2, 8, 10].map(function (i) {
    ...
    function getBox(boardArray, i) {
        var boxCol = Math.floor(i / 2) % 2;
        var boxRow = Math.floor(i / 8);
        var startingIndex = boxCol * 2 + boxRow * 8;
        return [0, 1, 4, 5].map(function (num) {
